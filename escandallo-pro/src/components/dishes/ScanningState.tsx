...
export function ScanningState({ image, pages, ai, startedAt, onRetry }: { image?: Blob; pages: number; ai: boolean; startedAt: string; onRetry: () => void }) {
    ...
    <Card className="hero-mesh overflow-hidden" padded={false}>
      ...
      <div className="grid gap-6 p-5 sm:p-8 md:grid-cols-[minmax(0,320px)_minmax(0,1fr)] md:items-center">
        ...
        <div>
          ...
          <div className="mt-5 h-1.5 w-full max-w-md overflow-hidden rounded-full bg-line" role="progressbar" aria-label="Leyendo la carta" aria-busy="true">
          ...
          {slow && (
            <div className="mt-5 flex flex-col gap-2 rounded-xl border border-warn/30 bg-warn-soft p-3 text-sm text-ink-2 sm:flex-row sm:items-center sm:justify-between">
              <span>Está tardando más de lo normal. Si cerraste la app mientras leía, vuelve a lanzarlo.</span>
